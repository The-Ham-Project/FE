...
//           const currentPos = new window.kakao.maps.LatLng(lat, lon); // 마커가 표시될 위치를 geolocation으로 얻어온 좌표로 생성합니다
...
//           // marker.setMap(currentPos);
//           // circle.setMap(currentPos);
...
//     })();
//   }, []);
//   // 최초의 마커랑 서클 만들어놓고 set하는건 딴데서 하자
//   // 새로 만드는건 유쥬 이뻭뜨에서 하고 useState나 Ref에 저장을 해두자
//   // 2. 현재 위치
//   const getCurrentPosBtn = () => {
//     setIsLoading(true); // API 요청 전 로딩 상태를 true로 변경
//     navigator.geolocation.getCurrentPosition(
...
//     // circle.setMap(null); // 반경 지움
//     // marker.setMap(null); // 마커 지움 기존을 지우는게 아니라서 그런걸수도 useRef 써서 기존꺼를 기억하기 없애고 싶은 애한테 .setMap(null)하기
//     circle.setMap(map);
//     marker.setMap(map);
//     setIsLoading(false); // API 요청 후 로딩 상태를 false로 변경
